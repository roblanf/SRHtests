...
for f in glob.glob('/data/srh/SRHtests/datasets/*/alignment.nex'):
    ...
    for charset_name in dat.charsets.keys():                                 
        ...
        table = table.append({'dataset': os.path.basename(os.path.dirname(f)), 'charset': charset_name, 'length': length}, ignore_index=True) 

table.to_csv('/data/srh/tables/charset_length.csv')